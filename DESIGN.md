...
- **Section lineup (anchors)**: `#top` hero · `#search` filterable sample-home cards · `#neighborhoods` 7 area cards · `#buyers` 5 tap-to-expand step cards · `#sellers` Italiana ledger + "What's your home worth?" CMA form · `#about` portrait + 3 pillars · `#contact` charcoal band + light form card.
...
- **Compliance**: sample homes carry a visible `Sample` badge + footer sentence ("illustrative examples, not active listings"); home-worth report labeled CMA-not-appraisal; EHO + license + full disclaimer in footer; neighborhood median prices are marked placeholders — never invent figures.

React Bits in use: SplitText (hero h1), Reveal, Magnet, SpotlightCard (buyer steps, gold spotlight), ClickSpark (`#b3924f`). CountUp/TiltedCard currently unused. The CMA/home-worth form is BACK by John's explicit request (his 7/6 brief) after being removed earlier the same day.
